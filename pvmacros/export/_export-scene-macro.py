...
def dumpDataArray(datasetDir, dataDir, array, root={}, compress=True):
    ...
    if compress:
        with open(pPath, 'rb') as f_in, gzip.open(
            os.path.join(dataDir, pMd5 + '.gz'), 'wb'
        ) as f_out:
...
def writeDataSet(
    filePath, dataset, outputDir, colorArrayInfo, newDSName=None, compress=True
):
...
def generateSceneName():
    ...
    while os.path.isfile(
        os.path.join(ROOT_OUTPUT_DIRECTORY, '%s%s' % (sceneName, FILENAME_EXTENSION))
    ):
        counter += 1
    ...
    for rpIdx in range(renProps.GetNumberOfItems()):
        ...
        if hasattr(renProp, 'GetMapper'):
            ...
            if dataset and dataset.GetPoints():
                ...
                colorArrayInfo = {'colorArray': colorArray, 'location': arrayLocation}
