...
@smproxy.filter(
    name="PVGeoTensorMeshAppender", label=TensorMeshAppender.__displayname__
)
@smhint.xml(
...
@smproxy.filter(name="PVGeoTopoMeshAppender", label=TopoMeshAppender.__displayname__)
@smhint.xml(
    '''<ShowInMenu category="%s"/>
    <RepresentationType view="RenderView" type="Surface" />'''
    % MENU_CAT
...
if discretize_available:
    ...
    @smproxy.filter(name="PVGeoOcTreeAppender", label=OcTreeAppender.__displayname__)
    @smhint.xml(
        '''<ShowInMenu category="%s"/>
        <RepresentationType view="RenderView" type="Surface With Edges" />'''
        % MENU_CAT
...
@smproxy.writer(
    extensions="msh", file_description="UBC Tensor Mesh", support_reload=False
)
@smproperty.input(name="Input", port_index=0)
...
class PVGeoTopoReader(TopoReader):
    ...
    @smproperty.xml(
        _helpers.get_file_reader_xml(
            TopoReader.extensions, reader_description=TopoReader.description
        )
    )
    ...
    @smproperty.intvector(
        name="SkipRows", default_values=0, panel_visibility="advanced"
    )
    def set_skip_rows(self, skip):
        TopoReader.set_skip_rows(self, skip)

    @smproperty.stringvector(
        name="Comments", default_values="!", panel_visibility="advanced"
    )
...
class PVGeoGravObsReader(GravObsReader):
    ...
    @smproperty.xml(
        _helpers.get_file_reader_xml(
            GravObsReader.extensions, reader_description=GravObsReader.description
        )
    )
    def AddFileName(self, filename):
...
if discretize_available:
    ...
    @smproxy.reader(
        name="PVGeoDiscretizeMeshReader",
    ...
    class PVGeoDiscretizeMeshReader(DiscretizeMeshReader):
        ...
        @smproperty.xml(
            _helpers.get_file_reader_xml(
                DiscretizeMeshReader.extensions,
                reader_description=DiscretizeMeshReader.description,
            )
        )
        def AddFileName(self, filename):
