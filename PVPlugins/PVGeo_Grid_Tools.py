...
@smproxy.filter(
    name='PVGeoReverseImageDataAxii', label=ReverseImageDataAxii.__displayname__
)
...
@smproxy.filter(
    name='PVGeoTranslateGridOrigin', label=TranslateGridOrigin.__displayname__
)
@smhint.xml('<ShowInMenu category="%s"/>' % MENU_CAT)
@smproperty.input(name="Input", port_index=0)
@smdomain.datatype(dataTypes=["vtkImageData"], composite_data_supported=True)
...
class PVGeoSurferGridReader(SurferGridReader):
    ...
    @smproperty.xml(
        _helpers.get_file_reader_xml(
            SurferGridReader.extensions, reader_description=SurferGridReader.description
        )
    )
...
@smproxy.writer(
    extensions="grd", file_description="Surfer Grid (ASCII)", support_reload=False
)
@smproperty.input(name="Input", port_index=0)
@smdomain.datatype(
    dataTypes=["vtkImageData", "vtkMultiBlockDataSet"], composite_data_supported=True
)
...
class PVGeoEsriGridReader(EsriGridReader):
    ...
    @smproperty.xml(
        _helpers.get_file_reader_xml(
            EsriGridReader.extensions, reader_description=EsriGridReader.description
        )
    )
    def AddFileName(self, filename):
...
class PVGeoLandsatReader(LandsatReader):
    ...
    @smproperty.xml(
        _helpers.get_file_reader_xml(
            LandsatReader.extensions, reader_description=LandsatReader.description
        )
    )
    def AddFileName(self, filename):
