...
class TestEsriGridReader(TestBase):
    ...
    def setUp(self):
        ...
        self.filename = os.path.join(os.path.dirname(__file__), 'data', 'vd2.asc')
    ...
    def test_read(self):
        ...
        self.assertEqual(ny, 9)
        dx, dy, dz = img.GetSpacing()
        self.assertTrue(100.0 == dx == dy == dz)
        ox, oy, oz = img.GetOrigin()
        self.assertEqual(ox, 0.0)
        self.assertEqual(oy, 0.0)
        self.assertEqual(oz, 0.0)
        # Check that NaNs are correct
        nans = np.argwhere(np.isnan(img["Data"]))
        truth = np.array([[22], [27], [32]])
        self.assertEqual(len(nans), 3)
        self.assertTrue(np.allclose(nans, truth))
        return
