Name = 'ReadUBCMesh2D'
Label = 'Read UBC Mesh 2D Two-File Format'
FilterCategory = 'PVGP Readers'
Help = 'UBC Mesh 2D models are defined using a 2-file format. The "mesh" file describes how the data is descritized. The "model" file lists the physical property values for all cells in a mesh. A model file is meaningless without an associated mesh file. Default file delimiter is a space character.'

NumberOfInputs = 0
OutputDataType = 'vtkRectilinearGrid'
Extensions = 'mesh msh dat'
ReaderDescription = 'PVGP: UBC Mesh 2D Two-File Format'
...
def RequestData():
    from PVGPpy.read import ubcMeshData2D
    import os
    ...
    ubcMeshData2D(FileName_Mesh, FileName_Model, dataNm=Data_Name, pdo=pdo)

def RequestInformation():
    from paraview import util
    from PVGPpy.read import ubcExtent2D
    if FileName_Mesh == 'absolute path':
        raise Exception('No mesh file selected. Aborting.')
    # Preview the mesh file and get the mesh extents
    ext = ubcExtent2D(FileName_Mesh)
